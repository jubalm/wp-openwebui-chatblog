...
**Version**: 3.0  
**Template**: Configuration templates and deployment patterns  
...
The platform configuration provides the following deployment pattern:
...
The platform configuration uses host-based routing with the LoadBalancer IP `<loadbalancer-ip>`:

- **Authentik:** `http://authentik.local` → `<loadbalancer-ip>`
- **OpenWebUI:** `http://openwebui.local` → `<loadbalancer-ip>`
- **WordPress Tenant 1:** `http://wordpress-tenant1.local` → `<loadbalancer-ip>`

To access these services, add the following entries to your `/etc/hosts` file:
```
<loadbalancer-ip> authentik.local
<loadbalancer-ip> openwebui.local
<loadbalancer-ip> wordpress-tenant1.local
```
...
curl -H "Host: authentik.local" http://<loadbalancer-ip>/
